...
A small Python tool that turns freeform fish survey notes into structured
data matching REEF's Volunteer Fish Survey Project (VFSP) format, using the
Claude API.
...
automates that re-entry step: it reads freeform notes and outputs a clean
CSV of species + abundance category, flagging anything it wasn't confident
about so a human can double-check before submitting.

## How it works

1. Takes a text file of freeform survey notes as input
2. Sends the notes to the Claude API with a system prompt describing REEF's
   abundance-category scale (Single = 1, Few = 2-10, Many = 11-100,
   Abundant = 100+)
...
5. Writes a CSV (`common_name, abundance_category, confidence, in_reef_id_list`)
6. Prints a summary and calls out any low-confidence entries for manual review
...
Output (`survey_output.csv`):
| common_name | abundance_category | confidence | in_reef_id_list |
|---|---|---|---|
| Blue Tang | Few | high | yes |
| Nurse Shark | Single | high | yes |
| Sergeant Major | Abundant | high | yes |
| Yellowtail Damselfish | Many | low | yes |
